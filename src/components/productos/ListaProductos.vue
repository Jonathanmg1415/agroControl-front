<template>
        ...
        <q-tr :props="props">
          ...
          <q-td auto-width>
            <div class="row q-pr-md">
              <div class="col">
                <q-btn
                  @click="verProducto(JSON.stringify(props.row.id))"
                  flat
                  bordered
                  class
                  size="10px"
              ...
              <div class="col q-pl-md">
                <q-btn
                  @click="eliminarProducto(JSON.stringify(props.row.id))"
                  flat
                  bordered
                  class
                  size="10px"
                  ...
                  icon="delete_forever"
                >
                  <q-tooltip class="bg-grey-6">Eliminar producto</q-tooltip>
                </q-btn>
              </div>
            </div>
          </q-td>
  ...
  const rowsExport = computed({
    get() {
      return productosStore.records.dataExportar;
    },
  });
  ...
  function eliminarProducto(idproducto) {
    ...
    $q.notify({
      progress: true,
      message: "Eliminar producto esta en contrucción ",
      icon: "information",
      color: "white",
      textColor: "orange",
    });
  ...
  function verProducto(idproducto) {
    ...
    $q.notify({
      progress: true,
      message: "Ver detalles del producto esta en contrucción ",
      icon: "information",
      color: "white",
      textColor: "orange",
    });
  ...
  /*async function exportarTablaXlsx() {
    try {
      ...
      await productosStore.exportarProductos(true);
      ...
      XLSX.writeFile(workBook, "Productos-" + formattedString + ".xlsx");
    } catch (error) {
      $q.notify({
        progress: true,
        message: "Error al momento de exportar los Productos. " + error,
        icon: "warning",
        color: "secondary",
        textColor: "white",
      });
