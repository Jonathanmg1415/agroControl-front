...
const RUTA_EXPORTAR_PRODUCTOS = '/productos/exportar';
...
export const useProductosStore = defineStore("productosStore", () => {
  ...
  async function exportarProductos(original) {
    const params = {
      params: {
        filter: filter.value,
        pagination: original
    ...
    try {
      const p = new Promise(async function (resolve, reject) {
        try {
          await axiosInstance
            .get(RUTA_EXPORTAR_PRODUCTOS, params)
            .then((response) => {
              if (response.data.ejecucion.respuesta.estado === "OK") {
                records.value.dataExportar =
                  response.data.ejecucion.datos.productos;
                resolve();
              } else {
                reject(new Error(response.data.ejecucion.respuesta.mensaje));
              }
